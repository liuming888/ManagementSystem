...
var monishujudizhi = path.resolve(__dirname, "在职人员模拟数据.txt");
fs.writeFileSync(monishujudizhi, "");
console.log("在职人员模拟数据.txt文件已经清空，下面我为你生成新的");
for(var i=0;i<1000;i++){
    var o={};
    var str="1234567890abcdefghijklmnopqrstuvwxyz",id='';
    for(var d=0;d<8;d++){
        id+=str[~~(Math.random()*str.length)];
    }
    o.id=id;
    o.ruzhishijian = Date.parse(new Date(new Date() - Random.integer(86400000, 86400000 * 365 * 5))).toString();
    o.danwei = Random.pick(['成型车间', '针车车间', '裁断车间', '财务部', '人事部', '业务部', '采购部', '品质部',"行政部"]);
    o.gangwei = Random.pick(["员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "员工", "副组长", "副组长", "副组长", "副组长", "副组长", "员工", "组长", "组长", "组长","副经理","经理"]);
    o.name = Random.cname();
