...
<template>
    <div class="bigbox">
        ...
        <div style="margin: 20px 0">
            ...
            <el-button type="primary" @click="luqubtn" size="small">录取已选</el-button>
            <div class="mohu">
                <p>
                    <span>{{this.spantxt}}</span>
                </p>
                <el-input class="sousuo" placeholder="请输入搜索内容" prefix-icon="el-icon-search" v-model.trim="sousuo" @blur="blurinput" size="small"></el-input>
            </div>
        </div>
        <el-table ref="multipleTable" :data="tableData3" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" @row-dblclick="tanchu($event.id)" size="mini" v-loading="loading2" element-loading-background="rgba(255, 255, 255, 0.8)">
            ...
            <el-table-column :prop="item.yin" :label="item.hanzi" width="240" v-for="(item,index) of titArr" :key="index">
            </el-table-column>
    ...
    export default {
      ...
      methods: {
        ...
        luqubtn() {
          ...
          this.$store.dispatch("DEL", this.multipleSelection); //删除
          this.$store.dispatch("LUQU", this.multipleSelection); //加入在职
          this.loading2 = true;
        }
      },
      watch: {
        ...
        "$store.state.mingxilist"(newval, oldval) {
          this.lists = this.$store.state.lists; //数据总数
          this.tableData3 = this.$store.state.mingxilist; //当前页的数据
          this.currentPage4 = 1; //默认第一页
          ~~((this.lists - 1) / this.bar) > 0
            ? (this.fenyebar = true)
            : (this.fenyebar = false); //就一页分页条不显示
          this.loading2 = false;
        ...
        sousuo(newval, oldval) {
          ...
          var arr3 = this.$store.state.mingxilist.filter((list, index) => {
            var arr = [];
